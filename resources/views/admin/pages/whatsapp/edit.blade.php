<x-admin.app-layout pageTitle="Edit Contact">
    ...
    <div class="pb-12">
        <div class="max-w-7xl mx-auto pt-6 px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.whatsapp.update', $contact->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-admin.text-input name="title" :value="old('title', $contact->title)" class="w-full mt-1" required />
                                    <x-admin.input-error :messages="$errors->get('title')" class="mt-1" />
                                </div>

                                <div>
                                    <x-admin.input-label for="subtitle" value="Subtitle (Optional)" />
                                    <x-admin.text-input name="subtitle" :value="old('subtitle', $contact->subtitle)" class="w-full mt-1" />
                            ...
                            <div class="mt-4">
                                ...
                                <x-admin.text-input name="phone" :value="old('phone', $contact->phone)" class="w-full mt-1 font-mono" required />
                            ...
                            <div class="mt-4">
                                ...
                                <textarea name="message" rows="2" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1">{{ old('message', $contact->message) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="icon" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm">
                                        @foreach(['shopping-bag', 'wrench', 'shield-check', 'message-circle', 'user', 'phone'] as $ico)
                                            <option value="{{ $ico }}" {{ old('icon', $contact->icon) == $ico ? 'selected' : '' }}>{{ ucfirst(str_replace('-', ' ', $ico)) }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="color" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm">
                                        @foreach(['green', 'blue', 'orange', 'red', 'purple'] as $col)
                                            <option value="{{ $col }}" {{ old('color', $contact->color) == $col ? 'selected' : '' }}>{{ ucfirst($col) }}</option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6 sticky top-6">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <x-admin.text-input type="number" name="order" :value="old('order', $contact->order)" class="w-full mt-1" min="0" />

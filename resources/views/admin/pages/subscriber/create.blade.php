                ...
                <form action="{{ route('admin.newsletter-subscribers.store') }}" method="POST">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <x-admin.text-input type="email" name="email" id="email" :value="old('email')" required autofocus 
                                class="w-full text-lg font-semibold" placeholder="customer@example.com" />
                        ...
                        <div>
                            ...
                            <select name="is_active" id="is_active" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm">
                                <option value="1" {{ old('is_active', 1) == 1 ? 'selected' : '' }}>Subscribed (Active)</option>
                                <option value="0" {{ old('is_active') == 0 ? 'selected' : '' }}>Unsubscribed (Inactive)</option>
                            </select>

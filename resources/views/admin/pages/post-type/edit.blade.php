<x-admin.app-layout pageTitle="Edit Post Type">
    ...
    <div class="pb-12" x-data="{ lang: 'en' }">
        <div class="max-w-7xl mx-auto pt-6 px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.post-types.update', $type->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6">
                            ...
                            <div class="space-y-4 min-h-[80px]">
                                <div x-show="lang === 'en'"
                                    ...
                                    <x-admin.text-input name="name[en]" :value="old('name.en', $type->getTranslation('name', 'en'))" class="w-full text-lg font-semibold mt-1" required />
                                ...
                                <div x-show="lang === 'id'" 
                                    ...
                                    <x-admin.text-input name="name[id]" :value="old('name.id', $type->getTranslation('name', 'id'))" class="w-full text-lg font-semibold mt-1" />
                    ...
                    <div class="space-y-6">
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6 sticky top-6">
                            <div class="flex items-center gap-2 mb-4 border-b border-gray-100 pb-2">
                                ...
                                <h3 class="font-bold text-gray-700">Settings</h3>
                            </div>
                            
                            <div class="space-y-4">
                                <div>
                                    <x-admin.input-label for="slug" value="Slug (Unique Key)" />
                                    <x-admin.text-input name="slug" :value="old('slug', $type->slug)" class="w-full mt-1 bg-gray-50 font-mono text-sm" required />
                                ...
                                <div>
                                    ...
                                    <select name="is_active" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm">
                                        <option value="1" {{ old('is_active', $type->is_active) == 1 ? 'selected' : '' }}>Active</option>
                                        <option value="0" {{ old('is_active', $type->is_active) == 0 ? 'selected' : '' }}>Inactive</option>
                                    </select>
                                </div>
                            </div>

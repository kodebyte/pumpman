                ...
                <div class="p-6 border-b border-gray-100">
                    <form method="GET" action="{{ route('admin.employees.index') }}" class="flex flex-col md:flex-row gap-4">
                        ...
                        <div class="w-full md:w-48">
                            <select name="role_id" class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" onchange="this.form.submit()">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <x-admin.th-sortable name="name" label="Employee" />
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Role</th>
                                <x-admin.th-sortable name="is_active" label="Status" />
                                <x-admin.th-sortable name="created_at" label="Joined Date" />
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                             <x-admin.avatar :name="$employee->name" class="h-10 w-10" />
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <x-admin.status-badge :active="$employee->is_active" />
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $employee->created_at->format('M d, Y') }}
                                        <span class="text-xs text-gray-400 block">{{ $employee->created_at->diffForHumans() }}</span>
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            <x-admin.btn-edit :href="route('admin.employees.edit', $employee->id)" />
                                            
                                            <x-admin.btn-delete :action="route('admin.employees.destroy', $employee->id)" :item="$employee->name" />
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <x-admin.empty-state colspan="5" message="No employees found." />
                            @endforelse
                        </tbody>
                    </table>
                </div>

                @if($employees->hasPages() || $employees->count() > 0)

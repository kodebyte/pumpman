                        ...
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6">
                            ...
                            <div class="space-y-6">
                                ...
                                <div x-show="bgType === 'video'" x-transition class="pt-4 border-t border-gray-100">
                                    <x-admin.input-label for="video" :value="__('Change Video (.mp4)')" />
                                ...
                                <div x-show="lang === 'en'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                                    <div class="space-y-5">
                                        <div>
                                            ...
                                            <x-admin.text-input name="tagline[en]" :value="old('tagline.en', $banner->tagline['en'] ?? '')" class="w-full text-sm" placeholder="e.g. NEW ARRIVAL" />
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Title (EN)')" />
                                            <x-admin.text-input name="title[en]" :value="old('title.en', $banner->title['en'] ?? '')" class="w-full text-lg font-bold" x-on:input="$refresh" placeholder="e.g. AIWA SPEAKER X-SERIES" />
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Subtitle (EN)')" />
                                            <textarea name="subtitle[en]" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm" rows="3" x-on:input="$refresh" placeholder="Short description in English...">{{ old('subtitle.en', $banner->subtitle['en'] ?? '') }}</textarea>
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Button Text (EN)')" />
                                            <x-admin.text-input name="cta_text[en]" :value="old('cta_text.en', $banner->cta_text['en'] ?? '')" class="w-full" placeholder="e.g. Shop Now" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div x-show="lang === 'id'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
                                    <div class="space-y-5">
                                        <div>
                                            ...
                                            <x-admin.text-input name="tagline[id]" :value="old('tagline.id', $banner->tagline['id'] ?? '')" class="w-full text-sm" placeholder="cth. PRODUK TERBARU" />
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Judul (ID)')" />
                                            <x-admin.text-input name="title[id]" :value="old('title.id', $banner->title['id'] ?? '')" class="w-full text-lg font-bold" x-on:input="$refresh" placeholder="cth. SPEAKER AIWA SERI-X" />
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Deskripsi (ID)')" />
                                            <textarea name="subtitle[id]" class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 mt-1 text-sm" rows="3" x-on:input="$refresh" placeholder="Deskripsi singkat dalam Bahasa Indonesia...">{{ old('subtitle.id', $banner->subtitle['id'] ?? '') }}</textarea>
                                        </div>
                                        <div>
                                            <x-admin.input-label :value="__('Teks Tombol (ID)')" />
                                            <x-admin.text-input name="cta_text[id]" :value="old('cta_text.id', $banner->cta_text['id'] ?? '')" class="w-full" placeholder="cth. Beli Sekarang" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="bg-white shadow-sm sm:rounded-xl border border-gray-100 p-6">
                            ...
                            <div>
                                ...
                                <x-admin.text-input name="target_url" :value="old('target_url', $banner->target_url)" required class="w-full text-sm" placeholder="/products/promo" />
                            </div>
                        </div>
                    </div>
                </div>

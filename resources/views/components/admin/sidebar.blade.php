<div x-data="{ menuSearch: '' }" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
...
>
    ...
    <div class="px-4 pt-4 pb-2">
        <div class="relative">
            ...
            <input 
                x-model="menuSearch" 
                type="text" 
                class="w-full py-2 pl-10 pr-4 text-xs bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:border-indigo-500 focus:bg-white transition-colors" 
    ...
    {{-- 3. NAVIGATION LIST --}}
    <div class="flex-1 flex flex-col overflow-y-auto py-2">
        <nav class="flex-1 px-2 space-y-1"
             x-effect="
                const term = menuSearch.toLowerCase();
                const links = $el.querySelectorAll('a');
                const sections = $el.querySelectorAll('.nav-section');
                ...
                // Filter Section Title (Hide if no links visible under it)
                if (term === '') {
            ...
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Product Catalog</p></div>
            ...
            {{-- GROUP: SALES & ORDERS --}}
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Sales & Orders</p></div>
            ...
            <x-admin.nav-link :href="route('admin.orders.index')" :active="request()->routeIs('admin.orders.*')">
                ...
                @if(isset($newOrdersCount) && $newOrdersCount > 0)
                    <span class="ml-auto bg-red-600 text-white py-0.5 px-2 rounded-full text-[10px] font-black animate-pulse">{{ $newOrdersCount }}</span>
                @endif
            </x-admin.nav-link>
            ...
            {{-- GROUP: CUSTOMER SERVICE (Baru: Gabungan fitur operasional support) --}}
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Customer Service</p></div>
            ...
            <x-admin.nav-link :href="route('admin.warranty-claims.index')" :active="request()->routeIs('admin.warranty-claims.*')">
                ...
                @if(isset($pendingWarrantyCount) && $pendingWarrantyCount > 0)
                    <span class="ml-auto bg-rose-600 text-white py-0.5 px-2 rounded-full text-[10px] font-black animate-pulse">{{ $pendingWarrantyCount }}</span>
                @endif
            </x-admin.nav-link>

            <x-admin.nav-link :href="route('admin.contacts.index')" :active="request()->routeIs('admin.contacts.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0l-8 4-8-4" /></svg>
                Inbox Messages
                @if(isset($unreadMessagesCount) && $unreadMessagesCount > 0)
                    <span class="ml-auto bg-indigo-600 text-white py-0.5 px-2 rounded-full text-[10px] font-black animate-pulse">{{ $unreadMessagesCount }}</span>
                @endif
            </x-admin.nav-link>
            ...
            {{-- GROUP: CONTENT & WEBSITE (Clients masuk sini) --}}
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Content & Website</p></div>
            ...
            <x-admin.nav-link :href="route('admin.product-highlights.index')" :active="request()->routeIs('admin.product-highlights.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-7.714 2.143L11 21l-2.286-6.857L1 12l7.714-2.143L11 3z" /></svg> Product Highlights
            </x-admin.nav-link>

            {{-- CLIENTS dipindah ke sini (sebagai aset visual / trust signal) --}}
            ...
            <x-admin.nav-link :href="route('admin.post-types.index')" :active="request()->routeIs('admin.post-types.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" /></svg> Post Types
            </x-admin.nav-link>
            
            <x-admin.nav-link :href="route('admin.faqs.index')" :active="request()->routeIs('admin.faqs.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9.247a3.75 3.75 0 015.544 0 3.75 3.75 0 01-1.114 6.22l-.234.09A2.25 2.25 0 0012 18v.75M21 12a9 9 0 11-18 0 9 9 0 0118 0zM12 21h.008v.008H12v-.008z" /></svg> FAQs
            </x-admin.nav-link>
            ...
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">User Management</p></div>
            ...
            <x-admin.nav-link :href="route('admin.newsletter-subscribers.index')" :active="request()->routeIs('admin.newsletter-subscribers.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg> Subscribers
            </x-admin.nav-link>
            ...
            {{-- GROUP: SYSTEM & SETTINGS (Couriers masuk sini) --}}
            <div class="pt-4 pb-1 nav-section"><p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">System & Settings</p></div>
            ...
            <x-admin.nav-link :href="route('admin.settings.index')" :active="request()->routeIs('admin.settings.*')">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg> General Settings
            </x-admin.nav-link>
        </nav>
